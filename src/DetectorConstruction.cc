...
//  Payload with 3 Compartments & Tagged Source
...
namespace grape
{
  ...
  //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   //----------------------------------
    ...
    // Solid Volume
    auto FEMBD_X = kAPB_X_Pitch;
    auto FEMBD_Y = kMIB_Ysize/4;
    auto FEMBD_Z = kMIB_Zsize;

    auto FEMBD_S = new G4Box(
                  "MIB_S",              // its name
                  FEMBD_X/2,              // its X dimension 
                  FEMBD_Y/2,              // its Y dimension
                  FEMBD_Z/2);             // its Z dimension
    
    // Logical Volume     
    auto FEMBD_LV = new G4LogicalVolume(
                  FEMBD_S,                // its solid
                  mPCB,                  // its material
                  "MIB_LV");            // its name

    auto FEMBD_Xpos = - kDAQ_Module_Zsize/2 + FEMBD_X/2 + kAPB_X_Pitch;
    auto FEMBD_Ypos = 0.0*mm;
    auto FEMBD_Zpos = APB_Zpos - APB_Z/2 - kMIB_Zoffset - FEMBD_Z/2; 
    // APB_Con_Zpos + APB_Con_Z/2 + kAPB_MIB_Zoffset + FEMBD_Z/2;
    auto FEMBD_Pos = G4ThreeVector( FEMBD_Xpos, FEMBD_Ypos, FEMBD_Zpos );

for (auto row = 0; row<APB_Xnum/2; row++) {
      FEMBD_Xpos =  ( row * (FEMBD_X + kAPB_X_Pitch)) - APB_Xpos - FEMBD_X;
      FEMBD_Pos = G4ThreeVector( FEMBD_Xpos, FEMBD_Ypos, FEMBD_Zpos );

      new G4PVPlacement(
                  0,                    // its rotation
                  FEMBD_Pos,              // its position
                  FEMBD_LV,               // its logical volume
                  "FEMBD_PV",             // its name
                  daq_module_LV,            // its mother  volume
                  false,                // no boolean operation
                  0,                    // copy number
    ...
    // Solid Volume
    auto DAQ_STK1_X = kMIB_Xsize/3;
    auto DAQ_STK1_Y = kMIB_Ysize/3;
    auto DAQ_STK1_Z = kMIB_Zsize;

    auto DAQ_STK1_S = new G4Box(
                  "MIB_S",              // its name
                  DAQ_STK1_X/2,              // its X dimension 
                  DAQ_STK1_Y/2,              // its Y dimension
                  DAQ_STK1_Z/2);             // its Z dimension
    
    // Logical Volume     
    auto DAQ_STK1_LV = new G4LogicalVolume(
                  DAQ_STK1_S,                // its solid
                  mPCB,                  // its material
                  "MIB_LV");            // its name

    auto DAQ_STK1_Xpos = - kDAQ_Module_Xsize/2 + DAQ_STK1_X/2 + kAPB_X_Pitch;
    auto DAQ_STK1_Ypos = - kDAQ_Module_Ysize/2 + DAQ_STK1_Y/2 + kAPB_Y_Pitch;
    auto DAQ_STK1_Zpos = APB_Zpos - APB_Z/2 - kMIB_Zoffset - DAQ_STK1_Z/2; 
    // APB_Con_Zpos + APB_Con_Z/2 + kAPB_MIB_Zoffset + DAQ_STK1_Z/2;
    auto DAQ_STK1_Pos = G4ThreeVector( DAQ_STK1_Xpos, DAQ_STK1_Ypos, DAQ_STK1_Zpos );

    new G4PVPlacement(
                  0,                    // its rotation
                  DAQ_STK1_Pos,              // its position
                  DAQ_STK1_LV,               // its logical volume
                  "DAQ_STK1_PV",             // its name
                  daq_module_LV,            // its mother  volume
                  false,                // no boolean operation
                  0,                    // copy number
                  fCheckOverlaps);      // checking overlaps 
    ...
    // Solid Volume
    auto DAQ_STK2_X = kMIB_Xsize/3;
    auto DAQ_STK2_Y = kMIB_Ysize/2;
    auto DAQ_STK2_Z = kMIB_Zsize;

    auto DAQ_STK2_S = new G4Box(
                  "MIB_S",              // its name
                  DAQ_STK2_X/2,              // its X dimension 
                  DAQ_STK2_Y/2,              // its Y dimension
                  DAQ_STK2_Z/2);             // its Z dimension
    
    // Logical Volume     
    auto DAQ_STK2_LV = new G4LogicalVolume(
                  DAQ_STK2_S,                // its solid
                  mPCB,                  // its material
                  "MIB_LV");            // its name

    auto DAQ_STK2_Xpos = - kDAQ_Module_Xsize/2 + DAQ_STK2_X/2 + kAPB_X_Pitch;
    auto DAQ_STK2_Ypos = - kDAQ_Module_Ysize/2 + DAQ_STK2_Y/2 + kAPB_Y_Pitch;
    auto DAQ_STK2_Zpos = DAQ_STK1_Zpos - kMIB_Zoffset*3 - DAQ_STK2_Z/2; 
    
    auto DAQ_STK2_Pos = G4ThreeVector( DAQ_STK2_Xpos, DAQ_STK2_Ypos, DAQ_STK2_Zpos );

    new G4PVPlacement(
                  0,                    // its rotation
                  DAQ_STK2_Pos,              // its position
                  DAQ_STK2_LV,               // its logical volume
                  "DAQ_STK2_PV",             // its name
                  daq_module_LV,            // its mother  volume
                  false,                // no boolean operation
                  0,                    // copy number
                  fCheckOverlaps);      // checking overlaps 
    ...
    // Solid Volume
    auto DAQ_STK3_X = kMIB_Xsize/2;
    auto DAQ_STK3_Y = kMIB_Ysize/2;
    auto DAQ_STK3_Z = kMIB_Zsize;

    auto DAQ_STK3_S = new G4Box(
                  "MIB_S",              // its name
                  DAQ_STK3_X/2,              // its X dimension 
                  DAQ_STK3_Y/2,              // its Y dimension
                  DAQ_STK3_Z/2);             // its Z dimension
    
    // Logical Volume     
    auto DAQ_STK3_LV = new G4LogicalVolume(
                  DAQ_STK3_S,                // its solid
                  mPCB,                  // its material
                  "MIB_LV");            // its name

    auto DAQ_STK3_Xpos = - kDAQ_Module_Xsize/2 + DAQ_STK3_X/2 + kAPB_X_Pitch;
    auto DAQ_STK3_Ypos = - kDAQ_Module_Ysize/2 + DAQ_STK3_Y/2 + kAPB_Y_Pitch;
    auto DAQ_STK3_Zpos = DAQ_STK2_Zpos - kMIB_Zoffset*2 - DAQ_STK3_Z/2; 
    
    auto DAQ_STK3_Pos = G4ThreeVector( DAQ_STK3_Xpos, DAQ_STK3_Ypos, DAQ_STK3_Zpos );

    new G4PVPlacement(
                  0,                    // its rotation
                  DAQ_STK3_Pos,              // its position
                  DAQ_STK3_LV,               // its logical volume
                  "DAQ_STK3_PV",             // its name
                  daq_module_LV,            // its mother  volume
                  false,                // no boolean operation
                  0,                    // copy number
                  fCheckOverlaps);      // checking overlaps 
